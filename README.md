...
- **Summary table** — duration, avg power, NP, max power, work (kJ), HR, cadence, speed, distance, ascent for each file
- **Delta metrics** (when exactly 2 files) — how the second file's numbers compare to the first in absolute units and percent
...
- **Time in Zones** — stacked-bar distribution of Coggan power zones (Z1-Z7), driven by an FTP input in the sidebar
- **Left / Right Power** — dual-sided power split per ride, with distinct blue (left) / red (right) colouring
- **Power vs Power scatter** (when 2 files) — every matched second plotted with a best-fit slope through origin. Headline metric: *"Meter B reads ±X% vs Meter A"*
...
- **FTP** — sets the zone boundaries for the Time in Zones chart
...
- **Sample rate.** Auto-detected from timestamp gaps. Non-1Hz files (e.g. Garmin "smart recording") get a warning, and MMP/smoothing windows are adjusted accordingly.
- **Work (kJ)** = integral of power over time ÷ 1000.
- **Best-fit slope** is a least-squares regression through the origin — it gives the systematic multiplicative offset between two meters.
